...
//Session starts 
session_start();
//including the authentication file
require_once '../config/connect.php';
//setting the email and password entered by user 
 if(isset($_POST) & !empty($_POST))
 {
    $email = mysqli_real_escape_string($connection, $_POST['email']);
	$password = md5($_POST['password']);
	//checks if the given email and password are present in the DB
	$sql = "SELECT * FROM admin WHERE email='$email' AND password='$password'";
	$result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
	$count = mysqli_num_rows($result);
	//if user is present then the count is set to 1 
	if($count == 1)
	{
	...
	{
		//if user is not a valid one, bootstrap error message is displayed in same login page 
...
<head>
	...
	<title>Shop Admin Login PHP E-commerce </title>
...
<div id="wrapper" class="wrapper">
		...
		<div class="container">
			<div class="row">
				<div class="col-md-3 col-xs-5 col-md-offset-5 logo">
					<img src="..\images\logo1.png" class="img-responsive" width="200" height="40"   alt="UNT Shopping Cart"/>
				</div>
	...
	<footer id="footer2">
		...
		<div class="footer-bottom container">
			<div class="row">
				<div class="col-md-6">
					<p>&copy; Copyright 2018. Cart Trail</p>
				</div>
				<div class="col-md-6">
