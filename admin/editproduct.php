<?php
   ...
   //Getting the Id from the products.php page for which product the details have to be updated
    ...
    //Setting the updated product details into parameteres of DB query
	if(isset($_POST) & !empty($_POST))
	{
        ...
        //If files are newly uploaded then validation is done
		...
		{
			//if no new file is  uploaded then same path is updated in DB
		...
		<?php if(isset($smsg)){ ?><div class="alert alert-success" role="alert"> <?php echo $smsg; ?> </div><?php } ?>
			...
			<form method="post" enctype="multipart/form-data">
			  ...
			  <input type="hidden" name="filepath" value="<?php echo $r['thumb']; ?>">
			    ...
			    <!--here the value is updated with already existing DB Name value-->
			    <input type="text" class="form-control" name="productname" id="Productname" placeholder="Product Name" value="<?php echo $r['name']; ?>">
			  </div>
			  <div class="form-group">
			  	<!--Even the text area is filled with previous value from product table-->
			  ...
			  <div class="form-group">
				...
				?>
				<!--The previously selected category is shown initialy when edit page is loaded-->
			  ...
			  <div class="form-group">
			    ...
			    <!--If image is set and not empty -->
			    <?php if(isset($r['thumb']) & !empty($r['thumb'])){ ?>
			    <br>
			        <!--If the image is already set then it is displayed with 100*100 thumbnail size-->
			        <!--Delete option is also provided with product id redirecting to delete page-->
